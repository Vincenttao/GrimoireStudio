...
class ScribeExtractor:
    ...
    @staticmethod
    def extract_from_ir(
        ir_block: StoryIRBlock, entities: List[Entity]
    ) -> ScribeExtractionResult:
        """
        Extract state changes from an IR Block.

        Args:
            ir_block: The Story IR Block to analyze
            entities: Current entities in Grimoire (for reference)

        Returns:
            ScribeExtractionResult with delta updates
        """
        updates = []
        ...
        for update in delta_result.updates:
            ...
            if delta.memory_to_append:
                ...
                if len(memories) > max_memory_items:
                    entity.current_status.recent_memory_summary = memories[
                        -max_memory_items:
                    ]
