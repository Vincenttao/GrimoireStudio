...
async def get_entity(entity_id: str) -> Optional[Entity]:
    ...
    async with get_db_connection() as conn:
        cursor = await conn.execute(
            "SELECT * FROM entities WHERE entity_id = ? AND is_deleted = 0", 
            (entity_id,)
        )
...
async def update_entity(entity_id: str, update_data: dict) -> Optional[Entity]:
    ...
    # Handle nested fields if passed flatly, but typically we receive JSON patches.
    # For MVP, assume update_data matches top-level Entity keys
    ...
    async with get_db_connection() as conn:
        await conn.execute(
            '''
            UPDATE entities 
            SET type = ?, name = ?, base_attributes_json = ?, current_status_json = ?, updated_at = ?
            WHERE entity_id = ?
            ''',
            (
